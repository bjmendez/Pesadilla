	...
	void Update () {
		...
		if (Vector2.Distance (transform.position, target.position) < aggroDistance) {		//If Player is within aggroDistance of enemy
			if (Vector2.Distance (transform.position, target.position) > enemyDistance) {	//If Player is further than the distance the enemy stops moving
				if (kiteing > 0) {                                                          //After hiting the player, enemy will back away until it can hit again. Goes together for 100 frames
					transform.position = Vector2.MoveTowards (transform.position, target.position - new Vector3(2, 2, 0), enemySpeed * Time.deltaTime);	//Move enemy towards player
					kiteing--;
				} else {
					transform.position = Vector2.MoveTowards (transform.position, target.position, enemySpeed * Time.deltaTime);	//Move enemy towards player to hit again
				}
				if (enemyAnimator == 1) {
